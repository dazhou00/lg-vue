  ...
  function createElm (vnode: VNode, insertedVnodeQueue: VNodeQueue): Node {
    ...
    if (sel === '!') {
      ...
      if (isDef(hook)) {
        ...
        if (hook.insert) {
          // 将具有 insert 钩子函数的 vnode 存储到队列中，后续再触发执行
    ...
    for (; startIdx <= endIdx; ++startIdx) {
      ...
      if (ch != null) {
        if (isDef(ch.sel)) {
          ...
          rm = createRmCb(ch.elm!, listeners) // 返回删除 DOM 元素的函数
          for (let i = 0; i < cbs.remove.length; ++i) cbs.remove[i](ch, rm)
          const removeHook = ch?.data?.hook?.remove
          if (isDef(removeHook)) {
            // 如果用户 传入 remove 钩子函数，则需要用户手动调用 rm 删除 DOM 元素
            removeHook(ch, rm)
          } else {
            rm()
          }
